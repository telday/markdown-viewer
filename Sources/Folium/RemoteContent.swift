...
enum RemoteContent {
    ...
    private static func attributeReferencesRemoteContent(_ attribute: String, in html: String) -> Bool {
        // `\b` before the name keeps `src=` from also matching inside
        // `srcset=` (the literal substring "src" occurs at its start) —
        // `srcset` is scanned in its own pass instead. Quoted with either
        // straight-quote style raw HTML in a document could use.
        let pattern = "\\b\(attribute)\\s*=\\s*(?:\"([^\"]*)\"|'([^']*)')"
        // The pattern above is built from a fixed, known-good set of
        // attribute names (loadingAttributes), never from document content,
        // so it always compiles; there is nothing here a test could feed it
        // to make this fail.
        guard let regex = try? NSRegularExpression(pattern: pattern, options: [.caseInsensitive]) else {
            return false
        }
        let range = NSRange(html.startIndex..., in: html)
        return regex.matches(in: html, range: range).contains { match in
            [1, 2].contains { groupIndex in
